...
## ✨ Новое в версии 1.10.1

- **Стабильность режима отладки** — Precision float16 для предотвращения OOM сбоев
- **Надёжность Streamlit** — Лимиты перезагрузки и полная диагностика сбоев
- **Исправления состояния сессии** — Корректная инициализация имени модели LLM
- **Обнаружение окружения** — Правильная обработка переменной RAG_DEBUG_MODE
- **Сохранение UI** — Интерфейс streamlit остаётся активным после обработки запроса
- **Выбор модели** — Быстрая отладочная модель с поддержкой embedding_dim getter

## ✨ Новое в версии 1.10.0

- **Комплексная система логирования для Process Mining** (3 фазы):
  - Контекстное логирование с информацией о модуле/классе/методе
  - Структурированное JSON логирование для анализа процессов
  - Трассировка запросов на основе Request ID для полной видимости пути выполнения
  - Отслеживание памяти на уровне каждого этапа (RAM и GPU)
- **Дашборд метрик** (Streamlit):
  - Интерактивная визуализация временной шкалы этапов пайплайна
  - Анализ длительности этапов и распределение производительности
  - Фильтрация логов в реальном времени и экспорт в Excel
- **Инструменты анализа производительности**:
  - `BottleneckAnalyzer` — Автоматическое обнаружение узких мест с рекомендациями
  - `MetricsExporter` — Экспорт метрик в DataFrame и Excel
  - Система бенчмарков для сравнения нескольких запусков с обнаружением регрессий
- **18 новых юнит тестов** (158 всего проходят):
  - Полное тестовое покрытие компонентов logging_utils
  - Интеграционные тесты для логирования пайплайна
  - Проверка отслеживания памяти и экспорта метрик
- **Документация** (`LOGGING_GUIDE.md`):
  - Полное руководство из 442 строк с примерами кода
  - Паттерны интеграции с RAG пайплайном
  - Best practices и рекомендации по решению проблем
