...
- **Интеграционные тесты** (17 новых тестов): Полное тестовое покрытие RAG пайплайна:
  - 7 тестов для API RAGPipeline (`test_rag_pipeline_query.py`)
  - 10 тестов для CLI (`test_app_cli.py`)
  - `TESTING.md` с полной документацией по запуску тестов и отладке
- **RAGApp класс**: Унифицированный интерфейс командной строки с поддержкой JSON вывода
  - Полная поддержка командной строки: --mode query, --query, --documents, --k, --retrieval_type, --output
  - Структурированные результаты с метриками генерации и найденными документами
- **Улучшения RAG пайплайна**:
  - Исправлена задержка LLM (2s → 120s для локальных моделей)
  - Улучшена обработка абсолютных путей в data loader
  - Улучшено логирование инициализации вектор-сторе
- **Инфраструктура цикла отладки**: Система полуавтоматической отладки с process mining и event логированием
  - Система event логирования для инструментации RAG пайплайна
  - Анализ process mining (варианты, узкие места, обнаружение аномалий)
  - Workflow управления задачами с YAML схемой
  - Утилиты отладочного контекста и отслеживания шагов

### Предыдущее (v1.8.0)

- **Тестирование UI & Валидация** (44 теста): Полное тестовое покрытие компонентов Streamlit:
  - 19 тестов для `streamlit_app.py` (инициализация состояния, обработка запросов, статистика)
  - 25 тестов для `components.py` (ConfigModal, FileListPanel, DocumentViewer, HighlightedAnswer)
- **Валидация входных данных**: Строгая проверка параметров `chunk_size`, `embedding_model`, `selected_file` и номера страницы
- **Исправление race condition**: Исправлена проблема консистентности в `handle_user_query()` где сообщения пользователя добавлялись перед ответом
- **Улучшение обработки ошибок**: Лучшие сообщения об ошибках и отдельная обработка ошибок валидации

### Предыдущее (v1.7.0)

- **Поддержка GPU в Docker**: Multi-stage Dockerfile с опциональной CUDA для ускорения embeddings
- **Автоматическая настройка GPU**: Новый скрипт `setup.sh` для валидации драйверов NVIDIA (3-уровневая проверка)
- **Быстрая конфигурация**: Шаблон `.env.example` для инициализации проекта
- **Расширенная документация**: Новое руководство Troubleshooting и улучшенные инструкции по установке
